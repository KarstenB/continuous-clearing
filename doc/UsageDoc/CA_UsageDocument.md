...
- [Continuous Clearing Tool Execution](#continuous-clearing-tool-execution)
  ...
  - [SBOM Signing and Verification](#sbom-signing-and-verification)
    ...
    - [SIT.Scan](#SIT-Scan)
    - [SIT.Create](#SIT-Create)
    - [SIT.Upload](#SIT-Upload)
    - [Prerequisite](#prerequisite-2)
    - [SIT.Scan](#SIT-Scan-1)
    - [SIT.Create](#SIT-Create-1)
    - [SIT.Upload](#SIT-Upload-1)
- [SIT.Upload Release Execution](#SIT-Upload-release-execution)
...
* SIT.Scan
  * [npm/NuGet/Maven/Python/Conan/Cargo](../usagedocimg/SITScannpmnuget.PNG)
  * [Debian/Alpine](../usagedocimg/SITScandebianalpine.PNG)
  * [BasicSBOM](../usagedocimg/SITScanBasicSBOMflowdiagram.png)
 
* SIT.Create
  * [npm/NuGet/Maven/Python/Conan/Cargo](../usagedocimg/SITCreatenpmnuget.PNG)
  * [Debian](../usagedocimg/SITCreatedebian.PNG)
  * [Alpine](../usagedocimg/SITCreateAlpine.PNG)

* SIT.Upload
  * [npm/NuGet/Maven/Python/Conan/Cargo](../usagedocimg/SIT.Upload.PNG)
...
> **2. SIT.Create**
> - Processes the SBOM file (output from the SIT.Scan) to create missing components/releases in SW360 and link all components to the project within the SW360 portal. This executable also triggers the upload of components to Fossology and automatically updates the clearing state in SW360.
> 
`Note: Since the SIT.Scan generates an SBOM file with Dev dependencies and internal components, ensure the RemoveDevDependency flag is set to true when executing this DLL.`

> **3. SIT.Upload**
> - This DLL processes the CycloneDX BOM file generated by the SIT.Create. It targets components with a cleared status ("Report approved") and facilitates their transfer from a remote repository to the configured third-party repository in JFrog Artifactory. Additionally, it manages the transfer of development components from the remote repository to the designated development repository. Internal packages are relocated to the configured release repository.
...
* In order to run the SIT.Create.dll , execute the below command.

  **Example** : `docker run --rm -it -v /path/to/OutputDirectory:/mnt/Output -v /path/to/LogDirectory:/var/log -v /path/to/configDirectory:/etc/CATool ghcr.io/siemens/continuous-clearing dotnet SIT.Create.dll --settingsfilepath /etc/CATool/appSettings.json`

### SIT.Upload

* SIT.Upload is ***not applicable for Debian and Alpine  type package*** clearance.

* In order to run the SIT.Upload dll , execute the below command.

  **Example** : `docker run --rm -it -v /path/to/OutputDirectory:/mnt/Output -v /path/to/LogDirectory:/var/log -v /path/to/configDirectory:/etc/CATool ghcr.io/siemens/continuous-clearing dotnet SIT.Upload.dll --settingsfilepath /etc/CATool/appSettings.json`
...
* In order to run the SIT.Scan.exe, execute the below command.

  **Example** : `SIT.Scan.exe --settingsfilepath /<PathToConfig>/appSettings.json`

* **With SBOM Signing enabled** (credentials from appSettings.json or command line parameters):

  **Example** : `SIT.Scan.exe --settingsfilepath /<PathToConfig>/appSettings.json --SbomSigning:SBOMSignVerify true --SbomSigning:KeyVaultURI "https://your-keyvault.vault.azure.net/" --SbomSigning:CertificateName "your-certificate" --SbomSigning:ClientId "your-client-id" --SbomSigning:ClientSecret "your-client-secret" --SbomSigning:TenantId "your-tenant-id"`

### SIT.Create

* In order to run the SIT.Create.exe, execute the below command.

  **Example** : `SIT.Create.exe --settingsfilepath /<PathToConfig>/appSettings.json`

### SIT.Upload

* SIT.Upload is ***not applicable for Debian and Alpine  type package*** clearance.

* In order to run the SIT.Upload exe, execute the below command.

  **Example** : `SIT.Upload.exe --settingsfilepath /<PathToConfig>/appSettings.json`

</details>

# SIT.Upload Release Execution

By default, the JFrogDryRun is set to True. This configuration is designed for the routine execution of the SIT.Upload on a daily basis during the project's development phase. The primary objective is to continuously verify the accuracy of component paths and permissions before actual operations.
When the JFrogDryRun is set to False, it indicates a shift towards deployment in a production environment. In this mode, the SIT.Upload is prepared for live operations, signaling the transition from the verification stage to the actual copy/move of components.

* In order to execute the tool in release mode we need to pass an extra parameter to the existing
  argument list.

  **Example** : docker run --rm -it -v /D/Projects/Output:/mnt/Output -v /D/Projects/DockerLog:/var/log -v /D/Projects/CAConfig:/etc/CATool ghcr.io/siemens/continuous-clearing dotnet SIT.Upload.dll --settingsfilepath /etc/CATool/appSettings.json --Jfrog:DryRun false

  or

  **Example** : SIT.Upload.exe --settingsfilepath /<PathToConfig>/appSettings.json -Jfrog:DryRun false

# How to handle multiple project types in same project

Incase your project has both npm/NuGet or other components it can be handled by merely running then SIT.Scan dll multiple times and generating a sinle SBOM.

### Example Steps for Execution:

1. Run the SIT.Scan dll with "**ProjectType**" set as "**npm**".

2. A cycloneDX  BOM will be generated in the output directory path that you have provided.

3. Next run the SIT.Scan dll with "**ProjectType**" set as "**NuGet**". In this run make sure that along with the usual arguments you also provide and additional argument "**--MultipleProjectType**" as True.
...
5. The remaining steps for the SIT.Create and SIT.Upload remains the same.
